...
export default {
  ...
  methods: {
    getAllPermisson() {
      // 查询所有权限
      this.api({
        url: '/v1/rbac/user/listAllPermission',
        method: 'get'
      }).then(data => {
    ...
    showUpdate($index) {
      ...
      this.tempRole.permissions = []
      for (let i = 0; i < role.menus.length; i += 1) {
...
<style>
#roleVue .requiredPerm {
  color: #ff0e13;
}

#roleVue .el-dialog__header {
  padding: 15px;
